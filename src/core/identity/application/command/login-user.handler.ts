...
export class LoginUserHandler implements ICommandHandler<LoginUserCommand> {
  constructor(private users: UserRepoPort, private hasher: HasherPort, private jwt: JwtPort, private bus: EventBusPort) { }
  async execute({ dto }: LoginUserCommand) {
    try {

        const user = await this.users.findByEmail(dto.email);
        if (!user) {
            throw err.notFound(ErrorCodes.IDENTITY.USER_NOT_FOUND, 'کاربر پیدا نشد');
        }
      ...
      await this.bus.publish('identity.user.logged_in', {
        userId: user.id, email: user.email, occurredAt: new Date().toISOString(),
      });
