...
export class BookingNotificationAdapter implements NotificationPort {
    ...
    private async getUserContact(userId: string) {
        const r = await this.ds.query(`SELECT email FROM users WHERE id=$1`, [userId]);
        return { email: r?.[0]?.email ?? '', phone: ''  };
    }

    async sendBookingCreated(userId: string, bookingId: string, expiresInSec: number): Promise<void> {
        const { email, phone } = await this.getUserContact(userId);
        try {
            await this.email.sendMail(email, 'رزرو موقت ایجاد شد', `رزرو شما ایجاد شد. تا ${expiresInSec} ثانیه فرصت پرداخت دارید.`);
        ...
        if (phone) {
            await this.sms.sendSms(phone, `رزرو ایجاد شد. ${Math.floor(expiresInSec/60)} دقیقه فرصت پرداخت دارید.`);
    ...
    async sendBookingConfirmed(userId: string, bookingId: string): Promise<void> {
        const { email, phone } = await this.getUserContact(userId);
        await this.email.sendMail(email, 'رزرو تایید شد', `رزرو ${bookingId} تایید شد.`);
        await this.logger.log({ userId, channel: 'email', template: 'booking_confirmed', payload: { bookingId }, success: true });
        if (phone) {
            await this.sms.sendSms(phone, `رزرو ${bookingId} تایید شد.`);
            await this.logger.log({ userId, channel: 'sms', template: 'booking_confirmed', payload: { bookingId }, success: true });
        }
    }

    async sendPaymentExpired(userId: string, bookingId: string): Promise<void> {
        const { email, phone } = await this.getUserContact(userId);
        await this.email.sendMail(email, 'لغو به‌علت عدم پرداخت', `رزرو ${bookingId} به دلیل عدم پرداخت لغو شد.`);
        await this.logger.log({ userId, channel: 'email', template: 'payment_expired', payload: { bookingId }, success: true });
        if (phone) {
            await this.sms.sendSms(phone, `رزرو ${bookingId} به دلیل عدم پرداخت لغو شد.`);
            await this.logger.log({ userId, channel: 'sms', template: 'payment_expired', payload: { bookingId }, success: true });
        }
    }
}
