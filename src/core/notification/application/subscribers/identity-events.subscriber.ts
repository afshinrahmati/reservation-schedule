...
export class IdentityEventsSubscriber implements OnModuleInit {
  constructor(
    private bus: EventBusPort, private logRepo: NotificationLoggerRepo,
    private email: MailerService, private sms: SmsService, private ds: DataSource,
  ) {}
  ...
  async onModuleInit() {
    await this.bus.subscribe('identity.user.registered', async (evt) => {
      ...
      try {
        await this.email.sendMail(email, 'خوش آمدید', 'ثبت‌نام شما با موفقیت انجام شد.');
        ...
        await this.logRepo.log({ userId: evt.userId, channel: 'email', template: 'user_registered', payload: { email }, success: false, error: String(e) });
      }
    });

    await this.bus.subscribe('identity.user.logged_in', async (evt) => {
      const email = await this.getUserEmail(evt.userId);
        await this.email.sendMail(email, 'خوش آمدید', 'وارد شما با موفقیت انجام شد.');
